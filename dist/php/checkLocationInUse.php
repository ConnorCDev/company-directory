...
try {
	...
	$query = $conn->prepare('SELECT `l`.`id` as `locationID`, `l`.`name` AS `locationName`, COUNT(`d`.`id`) as `departmentCount` FROM `location` `l` LEFT JOIN `department` `d` ON (`d`.`locationID` = `l`.`id`) WHERE `l`.`id` = ? GROUP BY `l`.`id`');
